...
import { ArrowLeft, Plus, Trash2, GripVertical, X, CheckCircle } from "lucide-react"
...
export default function EditSOPPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { sops, updateSOP } = useStore()

  const sop = sops.find((s) => s.id === id)

  const [title, setTitle] = useState("")
  const [category, setCategory] = useState("")
  const [steps, setSteps] = useState<{ instruction: string; notes: string }[]>([])
  ...
  useEffect(() => {
    if (sop) {
      ...
      setSteps(sop.steps.map((s) => ({ instruction: s.instruction, notes: s.notes || "" })))
      setTags(sop.tags)
    }
  }, [sop])
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-[family-name:var(--font-playfair)] text-2xl font-bold text-foreground">Edit SOP</h1>
            <p className="text-muted-foreground text-sm">Update your process documentation</p>
          </div>
        </div>
        {saved && (
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} className="bg-secondary/50" />
            </div>
